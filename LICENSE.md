# TLBCP Dual License

**Copyright (c) 2026 Elliot Readman (sintaxsaint)**

Each concept, technique, and piece of architecture in this repository is licensed from the date of its first commit to this repository. The commit timestamp serves as the official licence date for that specific content. New additions are covered automatically from their commit date. This licence applies to everything present in the repository at any given time.

This project is dual licensed depending on the nature of the user.

---

## Tier 1 — MIT License (Individuals, Students, Open Source Projects, Under £10,000 Annual Revenue)

Permission is hereby granted, free of charge, to any person or organisation meeting the above criteria obtaining a copy of this work and associated documentation to use, copy, modify, merge, publish, distribute, and build upon it, subject to the following conditions:

- The above copyright notice and this permission notice must be included in all copies or substantial portions of the work.
- Credit must be given to Elliot Readman (sintaxsaint) as the original author of the theoretical architecture.

THE WORK IS PROVIDED "AS IS", WITHOUT WARRANTY OF ANY KIND.

---

## Tier 2 — Commercial License (Organisations with £10,000+ Annual Revenue)

Any organisation with annual revenue exceeding £10,000 that uses, implements, or builds upon any architecture, technique, or concept described in this repository in a commercial product or service must obtain a commercial licence.

**The licence fee is charged annually and is completely recalibrated each year.** Each year is treated independently — the fee is calculated fresh against that year's total annual revenue, at whatever bracket that revenue falls into. Previous years have no bearing on the current year's fee.

**Fee structure:**

| Annual Revenue | Annual Licence Fee |
|---|---|
| £10,000 – £1,000,000 | 0.5% of that year's annual revenue |
| £1,000,000 – £10,000,000 | 1.0% of that year's annual revenue |
| £10,000,000 – £100,000,000 | 2.0% of that year's annual revenue |
| £100,000,000+ | 3.0% of that year's annual revenue |

**A single licence covers the entire repository.** Regardless of how many techniques, architectures, or concepts from this repository an organisation implements — whether one or all fourteen — only one annual licence fee is required. There is no additional charge per technique.

Credit must be given to Elliot Readman (sintaxsaint) as the original author in any product, publication, or service that incorporates anything from this repository.

To obtain a commercial licence, contact the author via GitHub: github.com/sintaxsaint

---

## Intent

This licence is designed so that this architecture remains permanently free and accessible to individuals, students, and the open source community, while ensuring that large organisations which profit from it contribute fairly to its creator.

Knowledge should be free. Profit derived from knowledge should be shared.
